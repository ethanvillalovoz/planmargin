...
uv run planmargin-waymax-smoke-test \
  --output experiments/stage-0/waymax-smoke-test.json
```
...
overlap, and offroad values, as expected for expert playback. The checked-in
[JSON report](waymax-smoke-test.json) contains small aggregate metric summaries,
environment versions, timings, hashes, and the permitted scenario identifier.
It contains no raw WOMD trajectory or map data.
...
counterfactual mutation is introduced.
